...
{
    public class ProutPipiIA : Client
    {
        ...
        public override void Tour(int numeroDuTour)
        {
            ...
            else // j'ai de la vie
            {
                ...
                else // On pille ^^ 
                {
                    ...
                    double moyenneScoreJoueurs = joueur.Average(r => r.Score) + 100;
                    if (me.Score > Constants.COUT_RECRUTEMENT + 100 && me.Score > moyenneScoreJoueurs)
                    {
                        command = new Recruter();
                    }
                    // Routes attaquables
                    else if (routesAttaquable.Count > aucuneRoute)
                    {
                        // On attaque la première route possible (changer pour prendre la meilleur, selon monstre etc..)
